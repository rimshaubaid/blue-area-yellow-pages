...
axios({
  method: "get",
  url: "http://localhost:5000/api/businesses/carrepair",
})
  .catch((error) => {
  ...
  const locationBool = (key) => {
    if (key === 1) {
      ...
      history.push({
        ...
        state: { detail: "F6", type: "car repair" },
      });
    } else if (key === 2) {
      setLocationServices("F7");
      history.push({
        pathname: "/filterByLoc",
        state: { detail: "F7", type: "car repair" },
      });
    } else if (key === 3) {
      setLocationServices("F8");
      history.push({
        pathname: "/filterByLoc",
        state: { detail: "F8", type: "car repair" },
      });
    } else if (key === 4) {
      setLocationServices("G6");
      history.push({
        pathname: "/filterByLoc",
        state: { detail: "G6", type: "car repair" },
      });
    } else if (key === 5) {
      setLocationServices("G7");
      history.push({
        pathname: "/filterByLoc",
        state: { detail: "G7", type: "car repair" },
      });
    } else if (key === 6) {
      setLocationServices("G8");
      history.push({
        pathname: "/filterByLoc",
        state: { detail: "G8", type: "car repair" },
      });
    }
  };
